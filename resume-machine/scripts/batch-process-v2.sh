...
for i in "${!entries[@]}"; do
  ...
  if [ "$role_template" = "auto" ]; then
    ...
    # Find corresponding job JSON (search in jobbankjobs directory)
    job_json_file=$(find jobbankjobs -name "*.json" -type f | xargs grep -l "\"title\".*\"$title\"" 2>/dev/null | head -1)
    
    if [ -n "$job_json_file" ]; then
      # Derive correlation file path from job JSON path
      correlation_file="${job_json_file%.json}_correlation.json"
      
      # Try to run correlator and get template from adapter
      if run_correlator_if_available "$job_json_file" "$correlation_file"; then
        ...
        if [ $? -eq 0 ] && [ -n "$template_data" ]; then
          ...
          # Extract domain from adapter output as suggested template
          adapter_domain=$(echo "$template_data" | jq -r '.domain_inference // "default"')
          
          if [ "$adapter_domain" != "default" ] && [ "$adapter_domain" != "fullstack" ]; then
            role_template="$adapter_domain"
            echo "✓ Using adapter-inferred template: $role_template"
          else
            # If adapter suggests default/fullstack, ask user or use default
            if [ "$AUTO_TEMPLATE" != "true" ]; then
              read -p "  Suggested template is general purpose. Use it anyway? (Y/n) " yn
              case "$yn" in
                [Nn]* ) 
                  read -p "  Enter custom template name (or press ENTER for 'default'): " custom_tmpl
                  if [ -n "$custom_tmpl" ]; then
                    role_template="$custom_tmpl"
                  else
            ...
            echo "✓ Using template: $role_template"
          fi
        else
          echo "⚠ Adapter failed; falling back to manual selection"
          if [ "$AUTO_TEMPLATE" != "true" ]; then
            read -p "  Enter template name (default: 'default'): " custom_tmpl
            role_template="${custom_tmpl:-default}"
          else
            role_template="default"
          fi
        fi
      else
        echo "⚠ Correlator unavailable; falling back to manual selection"
        if [ "$AUTO_TEMPLATE" != "true" ]; then
          read -p "  Enter template name (default: 'default'): " custom_tmpl
          role_template="${custom_tmpl:-default}"
        else
      ...
      echo "⚠ Job JSON not found; unable to correlate. Using default template."
      role_template="default"
    fi
  else
