...
# Compatibility shim — forwards to orchestration/batch-process.sh

HERE=$(cd "$(dirname "$0")" && pwd)
NEW="$HERE/orchestration/batch-process.sh"
if [ ! -f "$NEW" ]; then
  echo "Error: relocated batch-process.sh not found at $NEW"
  exit 1
fi
